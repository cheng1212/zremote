...
### BUG-39 流式回复时回看历史被顶走：「正在回复的框」无界增长挤压列表（用户痛点，2026-09-14）
- 现象：一边流式推消息一边往回滑，滑不动/停在历史区后被往最新端拉；
  「正在回复」的框随输出越来越长。
- 调研（GitHub 源码取证，Open WebUI/LobeChat/NextChat/Cline/Telegram/
  ChatGPT 交互基线）：四个项目全部收敛到同一结论——**视口不被拉走靠
  「布局总高恒定」（Telegram 钳制流式 cell 到剩余视口 / LobeChat spacer
  收缩吸收），而不是靠滚动补偿**；跟随与否靠「atBottom + 生成中 +
  无用户滚动意图」三条件每 tick 重判。调研同时确认我们架构的缺陷定位：
  `Column=[Expanded 列表, 流式面板]` 里面板无高度约束——内容每长一寸，
  Expanded 给列表的视口就被压矮一寸，停在历史区的用户被持续顶向最新端
  （「框越来越长」和「被往回拉」是同一个物理过程）。滑不动也是它：滑多
  少被顶回多少。
- 修复：
  ①`_StreamingPanel` 高度封顶（42% 屏高）+ 内部 `SingleChildScrollView`
  ——列表视口空间从此恒定（Telegram「流式槽位」的 Flutter 化；面板从
  0 长到上限期间用户通常在底部跟随无感，封顶后恒定）。
  ②面板内部跟尾：内容超出上限后每个 token tick 跳到最新输出（LLM UI
  惯例，ChatGPT/Open WebUI 同款 tail -f）；用户在面板内上滑回看（离底
  >48px）即停，贴底恢复。带同帧去重节流 + `isScrollingNotifier` 静默窗
  （拖拽/惯性中绝不 jump——跟随与手势对打是「滑不动」的另一来源）。
  ③`_InteractionsPanel` 同类封顶（70% 屏高 + 内部滚动）：多问题堆叠时
  不再无限吃列表空间。
  ④主列表侧零改动：FollowLock 锁存 + 锚行补偿（BUG-38）已覆盖「停在
  历史区钉住」「流式结束落行/面板卸载的过渡」——面板消失导致的列表
  扩张同样被锚行补偿钉住。
- 对照调研原则的自查：原则 1（钳制）✓、3（跟随闸门，主列表四重条件）
  ✓、4（阈值 48px 落在业界 5~56px 区间）✓、5（isScrolling 静默 + drag
  判定）✓、6（帧级节流）✓、7（落行过渡同帧 + 锚行补偿）✓、8（回底
  按钮=回底+恢复跟随）✓。
- 验证：analyze 0 问题 + 255 测试全过（2026-09-14）。待真机回归（用户
  手机）：①流式输出中往回滑，顺畅无顶推；②停在历史区等到回复完成，
  视口不动；③流式内容超长时面板封顶约 2/5 屏，内部自动跟最新输出，
  在面板内上滑可回看本条已输出部分；④流式结束面板消失瞬间视口不跳。
- 教训：「流式区出列表」只解决了列表内逐帧 reflow（BUG-27/28），没解决
  列表外部的布局挤压——**流式内容的增长必须被钳在一个恒定的布局预算
  内**，超出部分内部消化；任何「随内容生长」的面板都会把挤压转嫁给
  被它共享空间的兄弟节点。跟随类自动滚动的闸门必须是三条件同时成立，
  且滚动进行中一律静默。

### BUG-40 列表里有内容的会话打开却是「发第一条消息」（桌面端行窗口空，2026-09-14 取证）
- 现象：会话「调研优秀网页设计标准」（学习工作区）在列表里有预览、点开
  后整页空白，显示新会话引导「发第一条消息，开始这个会话」。
- 取证（桌面端在本机，logs + tasks-index.sqlite + cli/rollout 三方对质）：
  ①桌面日志 03:42:37 学习桥 attach 后，手机端订阅
  `subscribeConversationV4 OK`、多次 `conversationRowsRangeV4 OK`、用户点
  刷新后 `resyncConversationV4 OK`×6——**全部成功但行就是空**；
  ②桌面端 session-service 权威快照 `readSession`：该会话 `eventSeq=5915`
  （历史事件很多）但 `messageCount=1`（messageLimit:1 下的返回，与正常
  会话同形，单看无异常）；
  ③该会话正被**另一个 agent 在桌面端活跃使用**（browser-use 活动
  03:40-03:44，CLI 模型 IO 日志 cli/rollout/model-io-<sid>.jsonl 231KB
  且 03:59 仍在追加）——它的真实对话内容存在 **CLI 会话文件**里；
  ④tasks-index.sqlite 里该会话元数据齐全（searchable_text 有摘要、
  provider/mode 与正常会话无异，无可识别的"CLI 会话"标记）。
- 结论：**桌面端把 CLI 会话吸进了任务索引（列表可见、有预览），但其行
  流不归远端 conversation V4 行服务管辖 → 行窗口恒为空**。手机端订阅/
  拉行/重同步全部成功、如实渲染了 0 行——手机端无协议 bug；问题在桌面
  端索引与会话服务的数据脱节（另一 agent 占用可能加重了窗口不可见）。
- 手机端改进（本条落地部分）：空态二分。`rows0` 全空时查列表索引
  （`listedTasks` 的 `lastAssistantPreview`）与 `state.totalCount`：
  - 都没有 → 真新会话，保留原引导「发第一条消息，开始这个会话」；
  - 有 → **伪空**：文案改为「这个会话的内容暂时刷不出来——它可能正被
    桌面端使用中。点右上角 ↻ 重试，或到桌面端查看。」（图标
    cloud_off），不再误导用户以为会话是空的。
- 遗留（桌面端侧，手机改不了）：CLI 会话行流的远端暴露、索引与会话服
  务一致性——建议向桌面端反馈；会话被另一 agent 占用期间内容仍在持续
  产生，结束后再刷新可能恢复。
- 验证：analyze 0 问题 + 259 测试全过（2026-09-14）。真机验收：打开该
  会话应显示「内容暂时刷不出来」而非「发第一条消息」。
- 教训：空态是 UI 里最容易说谎的地方——「没有任何行」≠「没有内容」。
  判定空态要看**所有**知道真相的数据源（行窗口、totalCount、列表索引
  预览），说不出来龙去脉时宁可用中性文案。桌面端取证三件套（logs grep
  / tasks-index.sqlite 拷副本 / cli/rollout 文件时间戳）本次缺一不可。
...
### 改进 切会话改为乐观切换 + 本机历史顶上（用户裁定，2026-09-13）
- 背景：用户报「第一次点会话出不来，等一会、第二次才出来」。根因两段串行：
  ① 列表点卡片的 `onTap` **先 `await ensureTaskProject(t)` 再 push 路由**——
     在「全部对话」里点别的项目的会话必然要切桥（拆旧栈 + `openBridge` +
     重订阅索引，全是服务端往返），切桥那几秒界面毫无反馈；
  ② 进去之后才 `subscribeConversationV4`（代码注释实测：中位 4ms，**撞上
     排空暴发能到 22.6s**），又排在后台补拉后面。第二次点桥已挂好、队列已
     排空，所以显得"第二次才出来"。
- 用户裁定：**切会话要乐观**——点一下立刻进聊天页，切桥+订阅后台做；
  「能快速切进去发消息」是第一需求，历史/实时消息晚一点无所谓；**进去先用
  本机历史顶上**，允许与平板/PC 不一致（要准就刷新）；**发消息必须走服务端**。
- 修复：①`tasks_page` 点卡片不再等切桥，直接 push；②`openSession` 把项目
  对齐（`ensureTaskProject`）收进来在后台做，并做**并发去重**（
  `session_open_logic.sessionOpenPlan`）——原来列表点一次、聊天页 initState
  又发一次，两条 `subscribeConversationV4` 并发、后者覆盖前者且前者订阅泄漏；
  ③加 `_openGen` 代际：并发打开时先发的慢请求回来作废并 `dispose`，不再把
  `chat` 覆盖成上一个会话；④`ensureSessionReady` + `sessionReadyPlan`：发送
  前等就绪（切桥中途 `conv` 甚至为 null，直接发会拿旧桥把消息发到别的项目），
  已订上则一秒不等；⑤`_chatSnapshots` 本机历史快照（切走/切桥/断开前 `_stashChat`
  存一份**拷贝**，只留最近 3 个），聊天页 `_viewState` = 活订阅 ?? 快照，
  **行为判定（运行中/队列/压缩/发送键）一律仍以活订阅为准**，快照期顶部给
  一行「本机记录 · 正在同步…」。
- 验证：analyze 0 问题 + 234 测试全过（新增 session_open_logic 6 测）。
- 待真机验证：全部对话里点别的项目的会话，应立刻进页面并有本机记录可看；
  切进去马上打字发送应能正常发出（不报「桥未就绪」）。
- **补丁（同日，用户实测反馈「刚进去一屏会话订阅失败」）**：乐观切换把
  「切桥」挪进了 `openSession`，但 `chatLoading` 是进 `_openSession` 才置位的
  ——切桥那几秒里 `chatLoading=false` 而 `chat=null`，页面拿 `chat == null`
  当失败判据，于是进来先闪一屏「会话订阅失败 + 重试」。修：①`openSession`
  一开始就置 `chatLoading=true`；②新增 `chatError` 记录**真实失败原因**，
...
- 用户报障「历史拉不出来，好像只到上次切模型那儿」。探针实测
  （`test/manual_history_window_probe_test.dart`，ZREMOTE_PROBE_LINK 门控）
  查明真相反而不是"拉不动"：
  - 订阅快照**只给最近 60 行**（`rows=60`），`totalCount` 是**整个会话**
    （实测最大 4083 行），`firstRowId=1` 是会话首行（不是窗口首行）；
  - 翻页**是好的**：`loadOlder` 一轮一页顺序前进（4024→3964→3904…），
    `rowsRange(beforeRowId=54, limit=5)` 精确返回 `[49..53]`；
  - 真正的问题是**每轮只有 60 行**：4083 行的会话要 **68 轮**才够到最早，
    用户上滑几十次也到不了头 —— 观感就是"拉不出来"。
  - 附带实测：`limit=300` 桌面端**一轮 0 进账**（所以不能无脑开大页）。
- 用户裁定：一个会话几千条，**不要一进来就全拉**；首屏 100 条；要看全部给
  一个显式入口。
- 实现：①`_topUpInitialHistory`——订阅快照给 60，补一次到
  `kInitialHistoryRows=100` 就停（失败只记日志，不影响进来就能用）；
  ②列表顶部「还有 N 条更早 · 点这里拉取全部」入口（`history_logic.dart`
  纯函数出文案），点击走 `ZApp.pullAllHistory()` 循环翻页，顶部实时显示
  「正在拉取 2400/4083…」；③循环先试大页 200，**一轮 0 进账就退回 60 重试，
  连丢两轮收手**（`shouldKeepPulling`），防死循环也防大 limit 不被认。
