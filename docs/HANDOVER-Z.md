...
16. **`git commit -m` 的提交信息里不要用反引号**（2026-09-18 实测）：
    bash 会把 `` `…` `` 当命令替换**执行掉**，提交信息里那一段直接变空白，
    还会在 stderr 刷 `No such file or directory`。
    要写代码样式的路径就**用引号或纯文本**，别用反引号。
    （同因：`$` 也会被展开，写 `$VAR` 要转义或换写法。）
