...
### [2026-09-13] 乐观补偿层必须带对账出口——持久化墓碑让本机永久压住服务端
- 现象：多端（手机/平板/PC）会话列表互不一致；单端还出现过"会话全没了"
  （一次 listTasks 网络失败被吞成空列表）。
- 根因：删除墓碑被设计成**持久化、跨重连生效**——它本是修"删后复活"的
  补偿，但补偿没有退出机制：服务端拒删的会话被本机永久隐藏，各端各存一份
  墓碑必然各说各话；`loadTasks` 失败吞成空列表，把"拉取失败"伪装成
  "真的没有"。
- 解决：墓碑降级为「进行中 + 对账」乐观层（`_deletingTasks` 仅内存，
  `sweepDeletions` 裁决：服务端没有=确认；服务端还有且过宽限=恢复显示），
  启动即清旧库；失败保留缓存 + `tasksStale` 标记 + 退避重试 + UI 诚实横幅；
  归档集合断开/换工作区清空。commit 见 docs/BUGFIXES.md「多端一致性」条。
- 教训：①补偿机制必须内建**对账 + 退出**，否则它自己会变成新的分歧源——
  修"复活"的墓碑制造了"隐身"；②缓存可以有，但要在 UI 上诚实（陈旧标记），
  在数据上可被服务端纠正（下次成功拉取整表替换）；③"吞错成空"是最会伪装
  的 bug——空列表和"拉取失败"在用户眼里是两件事。
- 关联：lib/state/app_controller.dart `loadTasks`/`deleteTask`/`_deletingTasks`、
  lib/state/task_sort.dart `sweepDeletions`、lib/ui/tasks_page.dart 陈旧横幅、
  docs/BUGFIXES.md「多端一致性」
